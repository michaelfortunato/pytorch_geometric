...
class MeshToDualGraph(BaseTransform):
    ...
    def forward(self, data: Data) -> Data:
        ...
        if data.face.size(0) != 3:
            raise RuntimeError(
                f"data.face is shape ({data.face.shape}) which is not equal \
                to shape (3, num_faces).")
        ...
        visited_edges = {}
        dual_edges = torch.empty((2, data.face.size(1) * 3), dtype=torch.int64)
